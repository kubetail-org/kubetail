...
func loadLogConfig(cmd *cobra.Command) (*logFlags, []logs.Option) {
	// Get flags
	flags := cmd.Flags()
	...
	v.BindPFlag("commands.logs.head", flags.Lookup("head"))
	v.BindPFlag("commands.logs.tail", flags.Lookup("tail"))
	...
	return logFlags, streamOpts
}

func printLogs(rootCtx context.Context, cmd *cobra.Command, flags *logFlags, stream logs.Streamer) {
	...
	// Print header
	showHeader := flags.withTs || flags.withNode || flags.withRegion || flags.withZone || flags.withOS || flags.withArch || flags.withNamespace || flags.withPod || flags.withContainer

	if showHeader && !flags.hideHeader {
	...
	if flags.withCursors && !flags.follow && !flags.all {
		if flags.head && lastRecord != nil {
...
var logsCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		cm, err := k8shelpers.NewConnectionManager(env, k8shelpers.WithKubeconfigPath(flags.kubeconfigPath), k8shelpers.WithLazyConnect(true))
		cli.ExitOnError(err)
		...
		// output the logs
		printLogs(rootCtx, cmd, flags, stream)
...
func getTableWriterHeaders(flags *logFlags, sources []logs.LogSource) ([]string, []int) {
	headers := []string{}
	colWidths := []int{}
