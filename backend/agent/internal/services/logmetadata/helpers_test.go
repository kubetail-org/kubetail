...
func TestEventDebouncer(t *testing.T) {
	t.Run("1", func(t *testing.T) {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()

		debouncedFn := newEventDebouncer(ctx, 1*time.Second, func(ev fsnotify.Event) {
			fmt.Println(ev.Name)
		})

		debouncedFn(fsnotify.Event{Name: "1"})
		debouncedFn(fsnotify.Event{Name: "1"})
		debouncedFn(fsnotify.Event{Name: "1"})

		debouncedFn(fsnotify.Event{Name: "2"})
		debouncedFn(fsnotify.Event{Name: "2"})
		debouncedFn(fsnotify.Event{Name: "2"})
